...
class AiohttpSession(BaseSession):
    ...
    def build_form_data(self, data: dict) -> FormData:
        form_data = FormData(quote_fields=False)

        form_data.add_field('min_api_version', 1)
        form_data.add_field('sender', {'name': 'DE corp test bot', 'avatar': ''})
    ...
    async def make_request(self, bot, method: ViberMethod, timeout: int = None):
        ...
        try:
            async with session.post(
                url, data=json.dumps(method.dict()), timeout=self.timeout if timeout is None else timeout
            ) as resp:
